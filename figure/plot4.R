...
par(mfrow=c(2,2))
plot(ymd(data$Date)+hms(data$Time),data$Global_active_power,type="l",xlab="",ylab="Global Active Power")
plot(ymd(data$Date)+hms(data$Time),data$Voltage,type="l",xlab="datetime",ylab="Voltage")
...
plot(ymd(data$Date)+hms(data$Time),data$Global_reactive_power,type="l",xlab="datetime",ylab="Global_reactive_power")
dev.off()
